...
当前 [`v0.6.1` 候选](../../../release/v0.6.1-candidate.json)修订启动授权复用、
完整且适量的提示词/交接表达，以及配置合同的唯一必读入口。默认值、角色及权限边界不变，
新输入仍须独立验证与审查；参见[状态](STATE.md#current-v061-local-candidate)。
...
这不表示已提交、local release ready、安装或全局生效；其验收记录差异也已单独验证并通过 Planner 核对；这些历史结果不覆盖 v0.6.1 输入。

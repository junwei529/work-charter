...
当前 [`v0.6.2` 候选](../../../release/v0.6.2-candidate.json)补充 Agent 自行推导护栏的必要性说明，
并在辅助工作扩大时重新比较达到同一受保护用户结果的路线与剩余成本。
明确要求、默认值、schema 及生产安装行为不变；新输入仍须独立验证与审查，
参见[状态](STATE.md#current-v062-local-candidate)。
...
这不表示已提交、local release ready、安装或全局生效；其验收记录差异也已单独验证并通过 Planner 核对；这些历史结果不覆盖 v0.6.2 输入。
